...
metadata {
    definition (name: "InovelliDimmerRedSeriesDjdizzyd", namespace: "djdizzyd", author: "Eric Maycock", vid: "generic-dimmer", importUrl:"https://raw.githubusercontent.com/InovelliUSA/Hubitat/master/Drivers/inovelli-dimmer-red-series-lzw31-sn.src/inovelli-dimmer-red-series-lzw31-sn.groovy") {
        capability "Switch"
        capability "Refresh"
        capability "Polling"
        capability "Actuator"
        ...
        command "reset"
        command "notificationValue", ["number"]
...
def notificationValue(value) {
    def cmds = []
    log.debug "Notification value: ${value}"
    cmds << new hubitat.device.HubAction(command(setParameter(16, value, 4)), hubitat.device.Protocol.ZWAVE )
    return cmds
}
