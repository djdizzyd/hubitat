...
metadata {
    ...
    definition(name: "Ring Alarm Smoke/CO Listener", namespace: "djdizzyd", author: "Bryan Copeland") {
        ...
        fingerprint  mfr:"0346", prod:"0601", deviceId:"0101", inClusters:"0x5E,0x55,0x9F,0x85,0x59,0x80,0x70,0x5A,0x72,0x71,0x73,0x6C,0x86,0x84,0x7A", deviceJoinName: "Ring Alarm Flood Sensor"
    }
    preferences {
...
@Field static Map configParams = [
        1: [input: [name: "configParam1", type: "number", title: "Unsolicited Report Time (Heartbeat)", description: "seconds", range: "255..4200", defaultValue: 4200], parameterSize: 2],
        2: [input: [name: "configParam6", type: "enum", title: "Listening Mode", description: "", defaultValue: 2, options: [1: "Smoke Detector", 2: "Smoke/CO Detector"]], parameterSize: 1]
]
...
void pollDeviceData() {
    ...
    cmds.add(zwave.notificationV8.notificationGet(notificationType: 1, event: 0))
    cmds.add(zwave.notificationV8.notificationGet(notificationType: 2, event: 0))
...
void zwaveEvent(hubitat.zwave.commands.wakeupv2.WakeUpNotification cmd) {
    ...
    //sendHubCommand(new hubitat.device.HubAction(secureCommand(zwave.notificationV8.notificationGet(notificationType:5, event: 0, v1AlarmType:0)), hubitat.device.Protocol.ZWAVE))
    //sendHubCommand(new hubitat.device.HubAction(secureCommand(zwave.notificationV8.notificationSet(notificationType:5, event: 0)), hubitat.Protocol.ZWAVE))
...
void zwaveEvent(hubitat.zwave.commands.notificationv8.NotificationReport cmd) {
    ...
    log.debug "${cmd}"
    if (cmd.notificationType == 1) {
        switch (cmd.event) {
            case 0: //smoke - ENUM ["clear", "tested", "detected"]
            ...
            case 3:
                ...
                break
            case 4:
                log.warn "${device.displayName} Smoke Detector Replacement Required"
                break
            case 5:
                log.warn "${device.displayName} Smoke Detector Replacement Required, End-of-life"
                break
            case 6:
                log.warn "${device.displayName} Smoke Detector Alarm Silenced"
                break
            case 7:
                log.warn "${device.displayName} Smoke Detector Maintenance required, planned periodic inspection"
                break
            case 8:
                log.warn "${device.displayName} Smoke Detector Maintenance required, dust in device"
                break
            case 254:
                log.warn "${device.displayName} Smoke Detector Unknown event/state"
                break
        }
    } else if (cmd.notificationType == 2) {
        switch (cmd.event) {
            case 0: //carbonMonoxide - ENUM ["detected", "tested", "clear"]
                if (cmd.eventParametersLength > 0) {
                    switch (cmd.eventParameter[0]) {
                        case 1:
                            evt.name = "carbonMonoxide"
                            evt.value = "clear"
                            evt.isStateChange = true
                            evt.descriptionText = "${device.displayName} contact became ${evt.value}"
                            break
            ...
            case 3:
                ...
                break
            case 4:
                log.warn "${device.displayName} Carbon Monoxide Detector Replacement required"
                break
            case 5:
                log.warn "${device.displayName} Carbon Monoxide Detector Replacement required, End-of-life"
                break
            case 6:
                log.warn "${device.displayName} Carbon Monoxide Detector Alarm silenced"
                break
            case 7:
                log.warn "${device.displayName} Carbon Monoxide Detector Maintenance required, planned periodic inspection"
                break
            case 254:
                log.warn "${device.displayName} Carbon Monoxide Detector Unknown event/state"
                break
        }
